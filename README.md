...
| `DATABASE_URL` | Neon PostgreSQL connection string — free at [neon.tech](https://neon.tech) |
| `ADMIN_EMAIL` | your Google email — unlocks `/admin` dashboard |
...
**Database:** DiagramAI uses [Neon](https://neon.tech) (free PostgreSQL) for persistent storage — user accounts, history, and generation logs survive restarts and redeployments. Without `DATABASE_URL` it falls back to an in-container SQLite file (data lost on restart).
